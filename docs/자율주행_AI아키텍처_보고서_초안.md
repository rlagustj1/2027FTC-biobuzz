...
### 5.3 실로봇 자율 거동 — 인식·회피(반사층) 실측

계층 분리 원리를 실로봇에서 직접 구현·검증하였다. **인식**은 로봇 Control Hub에 연결한 웹캠으로
수행한다(VisionPortal + ColorBlobLocatorProcessor 색검출, `VisionColorTest`). 빨강=목표,
파랑=장애물을 실기에서 안정적으로 검출함을 확인하였다.

**장애물 회피(반사층)** 는 포텐셜 필드로 구현하였다(`AutoDriveAvoid`): 목표는 로봇을 당기고
(goto P제어), 카메라가 본 파란 장애물은 밀어낸다. 두 힘을 로봇 로컬 속도로 합성해, 목표로
가되 장애물을 옆으로 비켜 간다. **이 회피 연산은 AI(LLM)를 쓰지 않고 로봇 로컬에서 즉시**
수행된다 — 반사는 밀리초 단위여야 하므로 초 단위의 LLM을 기다릴 수 없기 때문이다.

- **결과**: 파란 장애물 접근 시 로봇이 옆으로 비켜 감을 실기로 확인. 다만 목표 인력과 장애물
  척력이 번갈아 작용해 **잔진동(경로가 오락가락)** 이 남는다. 이는 포텐셜 필드법의 잘 알려진
  **국소 진동/지역 최소점** 한계로, 진동 억제항(장애물 근접 시 목표의 측방 인력 감쇠)을 넣어
  완화했으나 완전 해소는 후속 튜닝 과제다. (온보드 로깅 `avoidlog.csv`로 계측 예정)

> **이 구현의 핵심 의의 — 다중 프로세싱의 실증**: 장애물 회피는 *같은 장애물 정보를 세 계층이
> 각기 다른 속도로* 처리하는 대표 사례다. ⚡**반사(회피)는 로봇이 AI 없이 즉시**, 👁️**인식은
> 카메라가 빠르게**, 🧠**전략(막히면 경로 재계획)은 PC의 LLM이 느리게**. *반사에 AI를 쓰지
> 않는 것 자체가* "왜 속도별로 계층을 나눠야 하는가"(H1·H2)를 실물로 증명한다 — 사람이 뜨거운
> 것에서 손을 떼는 반사(척수)와 그 뒤의 사고(뇌)가 분리된 것과 같다.

### 5.4 남은 측정 (H3·H4·H5)
- **H3**: RTT 분포(USB vs WiFi)로 "왜 실시간 제어는 로봇 로컬이어야 하는가" 정량 근거 — 로봇 필요.
- **H4**: `MockPerception`→`OpenCvPerception`/로봇 비전 등 실모델 교체로 인터페이스 모듈성 확인
  (본 프로젝트에서 인식을 카메라 실검출로 교체 완료 — 부분 달성).
...
| H4 | 모듈 교체가 인터페이스만으로 가능 | 코드 교체 | 🟢 **부분 달성**(인식을 카메라 실검출로 교체) |
...
- **실로봇 자율 거동을 구현**하였다: Control Hub 카메라로 목표(빨강)·장애물(파랑)을 인식하고,
  로봇이 목표로 가며 장애물을 **AI 없이 즉시(반사층)** 회피한다. *반사에는 AI를 쓰지 않고 전략에만
  쓰는* 이 구조 자체가 다중 프로세싱의 당위(H1·H2)를 실물로 증명한다.
- 관측된 회피 잔진동(포텐셜 필드 국소 진동)은 한계로 남으며, 온보드 로깅 기반 후속 튜닝 과제다.
- "느린 AI를 실시간 로봇에 안전하게 얹는" 일반적 설계 원리를, 소형 FTC 플랫폼에서 재현·검증하였다.
- 통신 RTT(H3)·안전 열화(H5) 정량화는 후속 검증 항목으로 남긴다.
